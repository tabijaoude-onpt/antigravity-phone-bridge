...
function isAgentManagerWindow(t) {
    ...
    if (title.endsWith(' - Antigravity')) return true;
...
// ── Send via xclip + xdotool (OS-level clipboard paste) ───────────────────────
...
const DISPLAY_ENV = { ...process.env, DISPLAY: ':99', HOME: '/home/tony', XAUTHORITY: '/home/tony/.Xauthority', PATH: '/usr/local/bin:/usr/bin:/bin' };
const XCLIP_BIN   = '/usr/bin/xclip';
const XDOTOOL_BIN = '/usr/bin/xdotool';

function xdotoolSend(text) {
    return new Promise((resolve) => {
        try {
            // Get all window IDs matching "Antigravity"
            const winResult = spawnSync(XDOTOOL_BIN, ['search', '--name', 'Antigravity'], {
                encoding: 'utf8', env: DISPLAY_ENV, timeout: 3000,
            });
            const winIds = (winResult.stdout || '').trim().split('\n').filter(Boolean);
            if (!winIds.length) throw new Error('No Antigravity window found by xdotool search');

            // Find the best window:
            // 1. Match by active CDP target title (most precise)
            // 2. Any window with a title longer than "antigravity" (real window, not helper)
            // 3. Fall back to first valid window
            let winId = null;
            ...
            for (const wid of winIds) {
                try {
                    const titleResult = spawnSync(XDOTOOL_BIN, ['getwindowname', wid], {
                        encoding: 'utf8', env: DISPLAY_ENV, timeout: 2000,
                    });
                    const title = (titleResult.stdout || '').trim();
                    if (!title) continue;

                    // Skip tiny helper processes (just lowercase "antigravity" with no workspace name)
                    if (title === 'antigravity') continue;
                    ...
                    // Otherwise take the first real window found
                    if (!winId) winId = wid;
                } catch (_) {}
            }

            if (!winId) throw new Error('No valid Antigravity window found (all were helper processes)');
...
// coordinates, uses xdotool to click precisely on the input, then types.
// Falls back to blind window-focus + type if CDP lookup fails.
async function directorSend(text, chatWsUrl) {
    // Find the right xdotool window ID (title-based, skipping helpers)
    const winResult = spawnSync(XDOTOOL_BIN, ['search', '--name', 'Antigravity'], {
        encoding: 'utf8', env: DISPLAY_ENV, timeout: 3000,
    });
    const winIds = (winResult.stdout || '').trim().split('\n').filter(Boolean);
    const windows = [];
    for (const wid of winIds) {
        const tr = spawnSync(XDOTOOL_BIN, ['getwindowname', wid], {
            encoding: 'utf8', env: DISPLAY_ENV, timeout: 2000,
        });
        const title = (tr.stdout || '').trim();
        if (title && title !== 'antigravity') windows.push({ wid, title });
    }
    if (!windows.length) return { success: false, error: 'No real Antigravity windows found' };

    // Pick target window: match by CDP target title if available, else first real window
    let target = null;
    if (chatWsUrl) {
        ...
        if (cdpTarget) {
            ...
            target = windows.find(w => w.title.includes(cleanTitle));
        }
    }
    if (!target) target = windows[0];

    const { wid: winId, title: winTitle } = target;
    console.log(`[/director] xdotool window: "${winTitle}" (wid=${winId})`);

    try {
        // Get window's screen position so we can convert page → screen coords
        const geoResult = spawnSync(XDOTOOL_BIN, ['getwindowgeometry', winId], {
            encoding: 'utf8', env: DISPLAY_ENV, timeout: 3000,
        });
        const geoMatch = (geoResult.stdout || '').match(/Position:\s*(\d+),(\d+)/);
        const winX = geoMatch ? parseInt(geoMatch[1]) : 0;
        const winY = geoMatch ? parseInt(geoMatch[2]) : 0;

        // Get bounding box of the chat input via CDP (if we have a wsUrl)
        let clickX = null, clickY = null;
        if (chatWsUrl) {
            try {
                const CHAT_INPUT_SCRIPT = `(() => {
                    const SELS = [
                        '[class*="max-h-"][class*="rounded"][class*="cursor-text"]',
                ...
                const pos = await new Promise((res) => {
                    ...
                    const t = setTimeout(() => { ws.close(); res(null); }, 4000);
                    ws.on('open', () => ws.send(JSON.stringify({id:1, method:'Runtime.evaluate', params:{expression:CHAT_INPUT_SCRIPT,returnByValue:true}})));
                    ws.on('message', d => { clearTimeout(t); try { res(JSON.parse(d)?.result?.result?.value); } catch(_){ res(null); } ws.close(); });
                ...
                if (pos && pos.x && pos.y) {
                    clickX = winX + pos.x;
                    clickY = winY + pos.y;
                    console.log(`[/director] clicking at screen (${clickX}, ${clickY})`);
                }
            } catch (_) {}
        }
...
app.get('/htop', (req, res) => {
    try {
        ...
        const out = execSync("ps aux --sort=-%cpu | head -21").toString();
...
app.get('/agent-proc-stats', (req, res) => {
    ...
    try {
        const out = execSync(`ps -p ${pidStr} -o pid=,pcpu=,pmem=,stat=,comm= --no-headers 2>/dev/null || true`).toString();
...
// All Antigravity windows — merges CDP targets (streamable) + xdotool scan (visible-only)
// across all known displays: :99 (virtual/CDP) and :10 (real desktop)
app.get('/all-windows', (req, res) => {
    ...
    // 2. Xdotool scan of real desktop (:10)
    const displays = [':10.0', ':10'];
    for (const disp of displays) {
        try {
            const widsRaw = execSync(
                `DISPLAY=${disp} xdotool search --name "Antigravity" 2>/dev/null || true`,
                { env: { ...process.env, DISPLAY: disp }, timeout: 3000, encoding: 'utf8' }
            ).trim();
            if (!widsRaw) continue;
            const wids = widsRaw.split('\n').filter(Boolean);
            for (const wid of wids) {
                try {
                    const title = execSync(
                        `DISPLAY=${disp} xdotool getwindowname ${wid} 2>/dev/null || true`,
                        { env: { ...process.env, DISPLAY: disp }, timeout: 2000, encoding: 'utf8' }
                    ).trim();
                    // Skip tiny utility windows (just "antigravity" lowercase)
            ...
            break; // found display, stop trying
        } catch (_) {}
    }
...
// Focus a specific xdotool window (for :10 non-CDP windows)
app.post('/focus-window', (req, res) => {
    const { wid, display } = req.body || {};
    if (!wid) return res.json({ success: false, error: 'No window ID' });
    try {
        const env = { ...process.env, DISPLAY: display || ':10.0' };
        execSync(`DISPLAY=${display || ':10.0'} xdotool windowactivate --sync ${wid}`, { env, timeout: 3000 });
...
app.post('/open-workspace', (req, res) => {
    ...
    try {
        // IMPORTANT: Do NOT pass --remote-debugging-port here.
        // VS Code IPC will detect the existing Antigravity instance via its Unix socket
        // (in XDG_RUNTIME_DIR) and open the new window within that process, so it
        // appears as a fresh CDP target on port 9222 automatically.
        const child = spawn('/usr/share/antigravity/antigravity',
...
app.get('/workspaces', async (req, res) => {
    try {
        const dirs = [
            '/home/tony/OnPoint-Flutter',
            '/home/tony/OnPoint',
            '/home/tony/Antigravity',
            '/home/tony/Antigravity Phone bridge',
            '/home/tony/Old-AG-Bridge',
            '/home/tony/Desktop/AI-Projects',
        ];
        const existing = [];
...
app.post('/ag-ciso-report/run', (req, res) => {
  const repo = req.body?.repo || '/home/tony/OnPoint-Flutter';
...
// Uses sudo (passwordless via /etc/sudoers.d/tony-reboot).
app.post('/system-reboot', (req, res) => {
    ...
    setTimeout(() => {
        const child = spawn('sudo', ['systemctl', 'reboot'], {
            detached: true,
            stdio: 'ignore',
            env: { ...process.env, PATH: '/usr/local/bin:/usr/bin:/bin', HOME: '/home/tony' },
        });
        child.unref();
    }, 1500);
});
...
app.post('/restart-bridge', (req, res) => {
    console.log('[/restart-bridge] Restarting onpoint-bridge service...');
    res.json({ success: true, message: 'Bridge restart initiated. Reconnect in ~10 seconds.' });
    setTimeout(() => {
        const child = spawn('sudo', ['systemctl', 'restart', 'onpoint-bridge'], {
            detached: true,
            stdio: 'ignore',
            env: { ...process.env, PATH: '/usr/local/bin:/usr/bin:/bin', HOME: '/home/tony' },
        });

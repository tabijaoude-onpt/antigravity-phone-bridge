...
// ── Send via PowerShell (OS-level clipboard paste) ───────────────────────
...
function xdotoolSend(text) {
    return new Promise((resolve) => {
        try {
            // Write to clipboard
            spawnSync(POWERSHELL_BIN, ['-NoProfile', '-Command', 'Set-Clipboard -Value $args[0]'], {
                input: text, encoding: 'utf8', timeout: 3000,
            });
            
            // Focus Window and Send Keys (^v and {ENTER})
            // We use a short inline C# snippet + SendWait in powershell for reliable Win32 window focus
            const psScript = `
Add-Type @"
  using System;
...
"@
$proc = Get-Process -Name "Antigravity" -ErrorAction SilentlyContinue | Where-Object {$_.MainWindowTitle -ne ""} | Select-Object -First 1
if ($proc) {
    ...
    throw "No Antigravity window found."
}
`;
            spawnSync(POWERSHELL_BIN, ['-NoProfile', '-Command', psScript], {
                encoding: 'utf8', timeout: 5000,
            });

            resolve({ success: true, method: 'powershell-paste' });
        } catch (e) {
            resolve({ success: false, error: e.message, method: 'powershell-type' });
        }
    });
}
...
// Focuses the main window via powershell, issues Ctrl+Alt+i to open AI panel,
// and pastes the required text.
...
if ($proc) {
    [Win32]::SetForegroundWindow($proc.MainWindowHandle)
    Start-Sleep -m 200
    [System.Windows.Forms.SendKeys]::SendWait('^%i') # Ctrl+Alt+i
    Start-Sleep -m 400
    [System.Windows.Forms.SendKeys]::SendWait('^v')
    Start-Sleep -m 50
    [System.Windows.Forms.SendKeys]::SendWait('{ENTER}')
...
`;
        ...
        return { success: true, method: 'director-powershell-paste' };
    } catch (e) {
...
async function ensureOllamaUp() {
    ...
    console.log('[ollama] Starting Ollama on-demand (Windows)…');
    // Using powershell or spawn directly on windows to start ollama server
    const start = spawn('ollama', ['serve'], { detached: true, stdio: 'ignore' });
    start.unref();
...
function scheduleOllamaShutdown() {
    ...
    ollamaIdleTimer = setTimeout(() => {
        ...
        spawnSync('taskkill', ['/F', '/IM', 'ollama.exe'], { encoding: 'utf8', timeout: 10000 });
...
app.get('/all-windows', (req, res) => {
    ...
    // 2. PowerShell scan of real desktop
    try {
        const psScript = `Get-Process -Name "Antigravity" -ErrorAction SilentlyContinue | Where-Object {$_.MainWindowTitle -ne ""} | Select-Object -ExpandProperty MainWindowTitle`;
        const widsRaw = execSync(
            `powershell.exe -NoProfile -Command "${psScript}" 2>nul || true`,
        ...
        if (widsRaw) {
            ...
            for (let i = 0; i < wids.length; i++) {
                ...
                if (!title || title.toLowerCase() === 'antigravity') continue;
...
// Focus a specific window (by exact title via PowerShell)
app.post('/focus-window', (req, res) => {
    const { wid } = req.body || {};
    if (!wid) return res.json({ success: false, error: 'No window ID (title)' });
    try {
        const psScript = `
Add-Type @"
  using System;
...
"@
$proc = Get-Process -Name "Antigravity" -ErrorAction SilentlyContinue | Where-Object {$_.MainWindowTitle -eq "${wid.replace(/"/g, '`"')}"} | Select-Object -First 1
if ($proc) { [Win32]::SetForegroundWindow($proc.MainWindowHandle) }
`;
        execSync(`powershell.exe -NoProfile -Command "${psScript}"`, { timeout: 3000 });
...
app.post('/open-workspace', (req, res) => {
    ...
    try {
        const agPath = fs.existsSync('C:\\Program Files\\Antigravity\\Antigravity.exe') 
            ? 'C:\\Program Files\\Antigravity\\Antigravity.exe'
            : os.homedir() + '\\AppData\\Local\\Programs\\Antigravity\\Antigravity.exe';

        const child = spawn(agPath,
...
app.get('/workspaces', async (req, res) => {
    try {
        ...
        const dirs = [
            join(home, 'OnPoint-Flutter'),
            join(home, 'OnPoint'),
            join(home, 'Antigravity'),
            join(home, 'Antigravity Phone bridge'),
            join(home, 'Old-AG-Bridge'),
            join(home, 'Desktop', 'AI-Projects'),
        ];
        const existing = [];
...
app.post('/restart-bridge', (req, res) => {
    ...
    setTimeout(() => {
        const child = spawn('powershell.exe', ['-NoProfile', '-Command', 'Restart-Service -Name "OnPointBridge" -Force'], {
            detached: true,
            stdio: 'ignore',
        });
        child.unref();
